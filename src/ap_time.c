...
int cmp_ftime(struct tm* t1, struct tm* t2, enum Passed_Time pt){
    ...
    double diff = difftime(t_first, t_last);
    ...
    case DAY:
        return diff >= SECS_IN_HOUR;
    case WEEK:
        return diff >= SECS_IN_HOUR * 7;
    case MONTH:
        return diff >= SECS_IN_HOUR * 30;
    default:
        report_fatal_error("in cmp_ftime(): no value for passed time was set or ");
    }
}
